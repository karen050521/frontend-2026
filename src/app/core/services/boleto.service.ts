...
import { Boleto, CreateBoletoDto, UpdateBoletoDto } from '../models/boleto.model';
...
export class BoletoService {
  private readonly apiUrl = getApiUrl('boletos');
  ...
  getBoletoById(id: number): Observable<Boleto> {
    return this.http.get<Boleto>(`${this.apiUrl}/${id}`).pipe(
      catchError(error => this.handleError(error))
    );
  }
