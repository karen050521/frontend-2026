...
import {
  ...
  DataTendencia,
} from '../../../core/services/reporte-incidentes.service';
...
export class TendenciaIncidentes implements OnInit {
  ...
  private procesarDataGrafico(data: DataTendencia[]): void {
    ...
    const categoriasConfig = [
      ...
      { key: 'retraso', label: '⏳ Retrasos', color: '#06b6d4', bg: 'rgba(6, 182, 212, 0.1)' },
      {
        key: 'pasajeros',
        label: '👥 Problemas Pasajeros',
        color: '#a855f7',
        bg: 'rgba(168, 85, 247, 0.1)',
      },
      { key: 'otro', label: '📁 Otros', color: '#22c55e', bg: 'rgba(34, 197, 94, 0.12)' },
    ];
    ...
    const datasets = categoriasConfig.map((cat) => {
      const dataMensual = listaMeses.map((mes) => {
        // Buscar si existe registro para este mes y esta categoría (normalizar a minúsculas para matcheo)
        const registro = data.find(
          (d) => d.mes === mes && d.tipo.toLowerCase() === cat.key.toLowerCase(),
        );
